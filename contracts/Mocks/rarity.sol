...
contract rarity is ERC721 {
    ...
    mapping(uint => uint) public classs;
    mapping(uint => uint) public level;
    
    event summoned(address indexed owner, uint classs, uint summoner);
    ...
    function tokenURI(uint256 _summoner) public view returns (string memory) {
        ...
        parts[1] = string(abi.encodePacked("class", " ", classes(classs[_summoner])));
